...
class Dialer:
    ...
    def _get_call_id(msisdn: str, modem: str, account: str) -> str:
        return hashlib.sha256('{}{}{}'.format(msisdn, modem, account).encode()).hexdigest()
    ...
    def _get_callback(self, endpoint_type: EndpointType, call_id=None):
        return {
            ...
            EndpointType.MODEM: lambda: self._modem_cb(
                self.lib,
                call_id=call_id,
                on_connect=self._on_modem_connect
            ),
        }[endpoint_type]()
    ...
    def add_account(
            self, alias: str, account_id: str, registration_uri: str,
            username: str, password: str, meta: typing.Dict = None
    ) -> 'Dialer':
        print('Adding account')
        assert not self.accounts.get(alias), 'Name clash'
        self.accounts[alias] = {
            'endpoint': self._create_endpoint(account_id, registration_uri, username, password),
            'meta': meta or {
    ...
    def add_modem(
            self, alias: str, account_id: str, local_modem_uri: str, registration_uri: str,
            username: str, password: str, meta: typing.Dict = None
    ) -> 'Dialer':
        print('Adding modem')
    ...
    def dial(self, msisdn: str, modem: str, account: str) -> types.Call:
        ...
        self._calls[_call_id] = types.Call(
            call_id=_call_id, call=call, msisdn=msisdn, modem=modem, account=account,
            call_endpoint=self.accounts[account]['endpoint'], modem_endpoint=self.modems[modem]['endpoint']
        )
